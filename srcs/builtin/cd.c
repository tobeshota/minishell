...
/*   Updated: 2023/11/24 19:54:05 by toshota          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	exec_cd(char **cmd, char ***envp)
{
	...
	if (get_path_from_cmd_arg(&path_from_cd, cmd, envp) == false)
	...
	if (getcwd(cwd, PATH_MAX) == NULL)
		return (free(path_from_cd), false);
	tmp = path_from_cd;
	path_from_cd = check_malloc(ft_strdup(cwd));
	if (update_envp(envp, "PWD=", path_from_cd) == false)
		return free(tmp), free(path_from_cd), false;
	return free(tmp), free(path_from_cd), true;
}
